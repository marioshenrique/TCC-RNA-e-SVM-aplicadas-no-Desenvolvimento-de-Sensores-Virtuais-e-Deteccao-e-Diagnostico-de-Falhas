...
dados = pd.read_csv("Simulacoes_12s_tratada_1_atraso.csv")
X_train = dados[['CB(k-1)', 'Tr(k-1)', 'Tc(k-1)', 'Tr0(k-1)', 'CA0(k-1)', 'Qc(k-1)', 'qr(k-1)']]
y_train = dados['CB(K)']

# 2. Carregar os dados de validação
dados_validacao = pd.read_csv("Simulacoes_12s_acuracia_tratada_1_atraso.csv")
X_validacao = dados_validacao[['CB(k-1)', 'Tr(k-1)', 'Tc(k-1)', 'Tr0(k-1)', 'CA0(k-1)', 'Qc(k-1)', 'qr(k-1)']]
...
model.save('modelo_treinado_12s_1_atraso.h5')
...
plt.title('RNA Tempo de Amostragem 12s e 1 Tempo de Atraso - Validação')
plt.savefig('grafico_dispersao_12s_1_atraso_validacao.png', dpi=300)
plt.show()