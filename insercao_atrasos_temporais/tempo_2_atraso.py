...
#df = pd.read_csv("Simulacoes_6s_acuracia_tratada.csv", nrows=4000000)
df = pd.read_csv("Simulacoes_12s_acuracia_tratada.csv")

# Inicializando o dataframe "base_dados"
colunas = ["CB(k-2)", "CB(k-1)", "Tr(k-2)", "Tr(k-1)", "Tc(k-2)", "Tc(k-1)", "Tr0(k-2)", "Tr0(k-1)", "CA0(k-2)", "CA0(k-1)", "Qc(k-2)", "Qc(k-1)", "qr(k-2)", "qr(k-1)", "CB(K)"]
...
base_dados.to_csv('Simulacoes_12s_acuracia_tratada_2_atraso.csv', index=False)